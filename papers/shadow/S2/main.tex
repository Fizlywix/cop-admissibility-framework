...
\begin{itemize}
  \item \textbf{Forward constraint:} COP-03 (Routed Tension),
  \item \textbf{Reverse constraint:} COP-06 (Effects-First Ontology).
\end{itemize}
...
\subsection{Forward: COP-03 Routed Tension}
...
\subsection{Reverse: COP-06 Effects-First Ontology}
...
Failure of routing becomes globally detectable as:
\begin{itemize}
  \item forced convergence,
  \item causal storytelling,
  \item retroactive justification.
\end{itemize}
...
\begin{itemize}
  \item If tension is being resolved rather than routed, stop (COP-03).
  \item If effects are being used to reconstruct causes, stop (COP-06).
...
\begin{figure}[h]
  \centering
  % Placeholder for routed-tension overlay on addressing diagram
  \includegraphics[width=0.92\linewidth]{figures/fig3-return-exposed_S2-overlay.pdf}
  \caption{S2 constraint overlay. The marked transition indicates routed coexistence under COP-03 and its reverse legibility constraint under COP-06.}
  \label{fig:s2-overlay}
\end{figure}

\begin{figure}[h]
  \centering
  % Placeholder for optional state-space or routing slice
