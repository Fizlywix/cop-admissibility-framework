...
\begin{itemize}
  \item \textbf{Forward constraint:} COP-05 (Stability Band),
  \item \textbf{Reverse constraint:} COP-13 (Responsibility Must Be Network-Routed).
\end{itemize}
...
\subsection{Forward: COP-05 Stability Band}
...
\subsection{Reverse: COP-13 Network-Routed Responsibility}
...
\begin{itemize}
  \item cascading failures beyond local thresholds,
  \item diffuse harm without a single origin,
  \item post-hoc blame reassignment,
  \item demands for centralized control.
\end{itemize}

These signals indicate that the stability band has been exceeded,
and responsibility has been misread.
...
\begin{itemize}
  \item If stability is treated as control, stop (COP-05).
  \item If responsibility is localized, stop (COP-13).
...
\begin{figure}[h]
  \centering
  % Placeholder for stability-band overlay
...
\begin{figure}[h]
  \centering
  % Placeholder for network responsibility slice
