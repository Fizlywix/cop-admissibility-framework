...
\begin{itemize}
  \item Constructive ($\rightarrow$): an operation that restricts future admissibility.
  \item Inferential ($\leftarrow$): an operation that excludes incompatible pasts.
\end{itemize}
...
\begin{enumerate}
  ...
  \item Write / Commit ($\leftrightarrow$ Backward Exclusion)
  \item Propagation ($\leftrightarrow$ Dependency Trace)
  \item Closure / Lock-in ($\leftrightarrow$ Invariant Inference)
\end{enumerate}
